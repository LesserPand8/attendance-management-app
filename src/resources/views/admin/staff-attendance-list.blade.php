...
<div class="content">
    <div class="list">
        <h2 class="list-title">{{ $staff->name ?? 'スタッフ' }}さんの勤怠</h2>
        <div class="list-month">
            <div class="month-navigation">
                <a href="/admin/attendance/staff/{{ $staff->id }}?month={{ \Carbon\Carbon::parse($currentMonth ?? now())->subMonth()->format('Y-m') }}" class="nav-button">
                    ←前月</a>
                <span class="current-month">
                    <img class="calendar-icon" src="{{ asset('storage/images/calendar.png') }}">
                    {{ \Carbon\Carbon::parse($currentMonth ?? now())->format('Y/n') }}
                </span>
                <a href="/admin/attendance/staff/{{ $staff->id }}?month={{ \Carbon\Carbon::parse($currentMonth ?? now())->addMonth()->format('Y-m') }}" class="nav-button">翌月→</a>
            </div>
        </div>
